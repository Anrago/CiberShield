...
import { IsBoolean, IsDate, IsInt, IsPositive, IsString } from "class-validator";
...
export class ExerciseResult {
    ...
    @ApiProperty({description: 'feedback must be a string',
        example: 'This is a feedback to exercise.'})
    feedback: string | undefined;

    @IsDate({message: 'createdAt must be a date'})
    @ApiProperty({description: 'createdAt must be a date',
